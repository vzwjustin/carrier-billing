...
## 2024-07-22 - Preventing RangeErrors in Math.max with large fleet arrays
**Learning:** Using the spread operator (`...`) with `Math.max` on dynamically-sized arrays (e.g., `Math.max(...array.map())`) can cause a `RangeError: Maximum call stack size exceeded` in V8 if the array is large (like enterprise fleet occurrence data). Additionally, `.map()` creates an unnecessary intermediate array.
**Action:** Always use `.reduce((max, item) => Math.max(max, item.value), -Infinity)` or a single-pass loop to track extreme values in unbounded data arrays.
