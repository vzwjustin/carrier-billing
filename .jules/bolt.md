...
## 2024-06-25 - Avoid spreading mapped arrays for min/max
**Learning:** Using `Math.max(...array.map())` and `Math.min(...array.map())` on dynamic arrays is an anti-pattern. It creates unnecessary intermediate arrays and risks `RangeError: Maximum call stack size exceeded` in V8 for large lists.
**Action:** Always replace spread maps for minimums and maximums with a single-pass `reduce()` or `for...of` loop initialized with `-Infinity`/`Infinity`.
