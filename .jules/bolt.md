...
## 2024-06-23 - Eliminating unnecessary chained filter-map logic (Rule Evaluations)
**Learning:** We continue to observe chained array iteration functions (like `.filter(fn1).filter(fn2).reduce().map().join()`) causing significant memory overhead by creating intermediate arrays. The `src/rules/definitions/` folder contains many rules that run for every line item on every bill, making these loops critical hot-paths.
**Action:** Found instances in rules (`src/rules/definitions/redundant-hotspot-addon.ts` and `src/rules/definitions/feature-first-seen.ts`). Rewriting these functions using a single-pass `for...of` loop skips creating extra arrays for `.filter()` allowing items to be evaluated and totalled immediately. Keep hunting for chained `.filter().reduce().map()` calls in frequently executed logic.
