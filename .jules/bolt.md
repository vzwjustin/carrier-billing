...
## 2024-06-25 - Avoid Math.max(...array) spreads
**Learning:** Using spread operators on potentially large dynamically-sized arrays (e.g., `Math.max(...array.map())`) allocates intermediate arrays and risks `RangeError: Maximum call stack size exceeded` in V8. It can also cause Cloudflare Workers / Next.js Edge Runtime CI build failures.
**Action:** Always refactor array spread operators inside `Math.max()` and `Math.min()` into a single loop or `reduce()` to track extreme values, even for smaller arrays, to guarantee environment compatibility and save memory allocations.
