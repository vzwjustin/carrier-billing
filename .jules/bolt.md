...
## 2024-06-25 - Avoid spreading mapped arrays into Math.max
**Learning:** Spreading a dynamically mapped array directly into `Math.max(...occurrences.map(o => o.monthly_cents))` causes multiple intermediate array allocations and can trigger a V8 `RangeError` if the initial array is extremely large.
**Action:** Refactor `Math.max(...array)` on potentially large inputs to calculate the max sequentially during a `for...of` loop or by using `.reduce()`.
