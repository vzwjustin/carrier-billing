...
## 2026-09-25 - Avoiding Math.max spreads on unbounded arrays\n**Learning:** Using `Math.max(...array.map())` on arrays that could scale significantly (e.g., fanning out a feature across thousands of lines in an enterprise account) creates an O(N) intermediate array allocation and risks throwing `RangeError: Maximum call stack size exceeded` in V8 when the spread limit is reached.\n**Action:** When tracking extremes like min/max on dynamically sized or potentially large datasets, prefer a single-pass `.reduce((max, x) => Math.max(max, x), -Infinity)` to maintain O(1) space complexity and avoid call stack overflow.
