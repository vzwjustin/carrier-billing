...
## 2024-06-11 - Single-pass filtering of Maps/Sets avoiding intermediate Arrays
**Learning:** Found an anti-pattern where a Map was converted into an Array and then `.filter()` was run on it, like `Array.from(map.entries()).filter(fn)`. This allocated a new large array just to iterate over it again.
**Action:** Use a `for...of` loop over `map.entries()` or `map.values()` and push to a new array conditionally. This skips one unnecessary full-array allocation and the double-loop traversal.
