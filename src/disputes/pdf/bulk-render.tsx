...
const SEVERITY: Record<
  BulkDisputeItemDisplay['severity'],
  ...
  return (
    <View style={styles.itemBlock} wrap={false}>
      ...
      <Text style={styles.itemMetaValue}>
        {item.affectedAccounts.length === 0
          ? '—'
          : item.affectedAccounts.join('  ·  ')}
      </Text>
      ...
      <Page size="A4" style={styles.page} wrap>
        ...
        <View style={styles.totalBlock} wrap={false}>
          ...
          <Text style={styles.totalValue}>
            {letter.totalRequestedCreditDisplay}
          </Text>
        ...
        <Text style={styles.sectionHeading}>
          {`Disputed items (${letter.items.length})`}
        </Text>
        ...
        <View style={styles.closing} wrap={false}>
          {letter.closingBlock.map((line, idx) => {
            const isName =
              idx === letter.closingBlock.length - 1 && line.length > 0;
            return (
              <Text
                key={idx}
                style={isName ? styles.closingNameLine : styles.closingLine}
              >
                {line.length === 0 ? ' ' : line}
              </Text>
            );
...
export async function renderBulkDisputePdf(
  letter: BulkDisputeLetter,
): Promise<Buffer> {
