...
export function compareBills(previous: ExtractedBill, current: ExtractedBill): AutopsyResult {
  ...
  // ⚡ Bolt: Single-pass iteration
  // Why: Replaces 3 chained `.filter().reduce()` passes and 1 `.map()` equivalent pass.
  // Impact: Reduces O(4n) traversal to O(n) and eliminates 3 intermediate array allocations.
  let disputable = 0;
  let optimization = 0;
  let unexplained = 0;
