...
export function compareBills(previous: ExtractedBill, current: ExtractedBill): AutopsyResult {
  ...
  // ⚡ Bolt: Consolidated 4 iterations into 1.
  // 💡 What: Replaced three `.filter().reduce()` chains and one `.map()` over `drivers` with a single loop.
  // 🎯 Why: Avoids iterating over the `drivers` array 4 separate times and allocating 3 intermediate arrays.
  // 📊 Impact: Reduces O(4N) iteration overhead to O(N) and decreases GC pressure by preventing intermediate array allocations.
  let disputable = 0;
  let optimization = 0;
  let unexplained = 0;
