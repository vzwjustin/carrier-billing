...
export const ROADMAP: RoadmapSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      p('Wire Verizon editor UI to load/save real data'),
      p('Add tRPC query to load T-Mobile bills from database'),
      p('Add tRPC mutation to save T-Mobile bill edits'),
      p('Wire T-Mobile editor UI to load/save real data'),
      p('Add tRPC query to load AT&T bills from database'),
      p('Add tRPC mutation to save AT&T bill edits'),
      p('Wire AT&T editor UI to load/save real data'),
      p('Create unified carrier bill editor interface with carrier selection'),
      p('Implement real carrier-specific optimization rules engine'),
      p('Add bill comparison view (current vs. optimized)'),
      p('Generate before/after savings visualization'),
      p('Export modified bill details (PDF/CSV)'),
      p('Integrate edited bills with audit findings'),
      p('Add manual bill entry interface for all carriers'),
      p('Create carrier-specific validation rules'),
      p('Build bill preview and confirmation flow'),
    ],
  },
];
