  ...
  evaluate: ({ bill, today }) => {
    ...
    bill.accounts.forEach((account, accountIndex) => {
      account.lines.forEach((line, lineIndex) => {
        ...
        // PERF: Loop directly to avoid intermediate arrays and spread operator call stack limits
        let dppMonthsLeft = -Infinity;
        for (const d of line.dpp_installments) {
          if (d.remaining_payments !== null && d.remaining_payments > 0) {
            if (d.remaining_payments > dppMonthsLeft) dppMonthsLeft = d.remaining_payments;
          }
        }
