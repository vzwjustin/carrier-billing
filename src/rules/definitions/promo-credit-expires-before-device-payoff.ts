  ...
  evaluate: ({ bill, today }) => {
    ...
    bill.accounts.forEach((account, accountIndex) => {
      account.lines.forEach((line, lineIndex) => {
        ...
        // ⚡ Bolt: Use a single-pass loop instead of mapped intermediate arrays and spread.
        let dppMonthsLeft = -Infinity;
        let hasRemainingTerms = false;
        for (const d of line.dpp_installments) {
          const n = d.remaining_payments;
          if (n !== null && n > 0) {
            hasRemainingTerms = true;
            if (n > dppMonthsLeft) {
              dppMonthsLeft = n;
            }
          }
        }
        if (!hasRemainingTerms) return;
        ...
        // ⚡ Bolt: Avoid spread operator on potentially unbounded arrays.
