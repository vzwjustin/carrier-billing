      ...
      account.lines.forEach((line, lineIndex) => {
        ...
        const dppMonthsLeft = line.dpp_installments.reduce((max, d) => {
          if (d.remaining_payments !== null && d.remaining_payments > 0) {
            return Math.max(max, d.remaining_payments);
          }
          return max;
        }, -Infinity);
        ...
        const soonestCyclesLeft = expiringCredits.reduce((min, c) => {
          return Math.min(min, creditCyclesLeft(c.expires_on as string));
        }, Infinity);
