  ...
  evaluate: ({ bill, today }) => {
    ...
    bill.accounts.forEach((account, accountIndex) => {
      account.lines.forEach((line, lineIndex) => {
        ...
        // ⚡ Bolt: Single-pass reduce avoids intermediate array allocation and spread operators
        const dppMonthsLeft = line.dpp_installments.reduce(
          (max, d) => (d.remaining_payments !== null && d.remaining_payments > 0 ? Math.max(max, d.remaining_payments) : max),
          -Infinity,
        );
        ...
        // ⚡ Bolt: Use reduce to avoid intermediate array allocation and spread operators
