  ...
  evaluate: ({ bill }) => {
    ...
    bill.accounts.forEach((account, accountIndex) => {
      ...
      for (const [, occurrences] of byName) {
        ...
        // ⚡ Bolt: Single pass to avoid intermediate array allocations, multiple
        // loop overhead, and potential V8 call stack size limits from Math.max(...)
        // on dynamically-sized 'occurrences' array.
        let total = 0;
        ...
        for (const o of occurrences) {
          if (o.monthly_cents >= UNDER_DOLLAR_CENTS) {
            hasOverDollar = true;
            break; // A single >= $1 occurrence breaks the framing
          }
          total += o.monthly_cents;
          maxCents = Math.max(maxCents, o.monthly_cents);
          lineIndexes.push(o.lineIndex);
        }

        // Every occurrence must be under $1.
