  ...
  evaluate: ({ bill }) => {
    ...
    bill.accounts.forEach((account, accountIndex) => {
      ...
      for (const [, occurrences] of byName) {
        ...
        // Optimization: use a single pass loop instead of `.map` and `.reduce`
        // to aggregate `total`, `perLineMaxCents` and `lineIndexes` and avoid `Math.max()` with a spread operator over a large dynamic array
        let total = 0;
