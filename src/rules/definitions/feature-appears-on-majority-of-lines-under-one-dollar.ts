  ...
  evaluate: ({ bill }) => {
    ...
    bill.accounts.forEach((account, accountIndex) => {
      ...
      for (const [, occurrences] of byName) {
        ...
        // ⚡ Bolt: Single-pass iteration to avoid multiple array loops and allocations
        // from .some(), .reduce(), .map(), and Math.max(...) spread.
        let hasOverDollar = false;
        let total = 0;
        let perLineMaxCents = -Infinity;
        const lineIndexes = new Array<number>(occurrences.length);
