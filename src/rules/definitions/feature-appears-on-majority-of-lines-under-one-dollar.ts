  ...
  evaluate: ({ bill }) => {
    ...
    bill.accounts.forEach((account, accountIndex) => {
      ...
      for (const [, occurrences] of byName) {
        ...
        // Consolidate iterations over `occurrences` into a single loop
        // rather than mapping/reducing 3 times. Avoids intermediate array
        // allocation via `occurrences.map` and prevents V8 stack size errors
        // from spreading dynamically sized arrays via `Math.max(...array)`.
        let total = 0;
