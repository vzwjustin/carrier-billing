  ...
  evaluate: ({ bill }) => {
    ...
    bill.accounts.forEach((account, accountIndex) => {
      ...
      for (const [, occurrences] of byName) {
        ...
        // ⚡ Bolt: Single-pass iteration to avoid intermediate array allocations
        // from .map() and prevent RangeError from spread operator on large arrays.
        let total = 0;
        let perLineMax = -Infinity;
        const lineIndexes: number[] = [];

        for (const o of occurrences) {
          total += o.monthly_cents;
          lineIndexes.push(o.lineIndex);
          if (o.monthly_cents > perLineMax) {
            perLineMax = o.monthly_cents;
          }
        }
