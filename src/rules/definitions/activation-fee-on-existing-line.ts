    ...
    bill.accounts.forEach((account, accountIndex) => {
      account.lines.forEach((line, lineIndex) => {
        ...
        findings.push({
          ...
          description: `This line carries an activation/connection-style charge (${names}, ${formatCents(total)}) despite having an established device-payment-plan history (at least one paid installment on file). These fees are almost always reversible on a single rep-call when the line is not actually newly activated.`,
          recommended_action:
