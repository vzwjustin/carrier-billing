...
type GenerateResponse = { ok: true; token: string } | { ok: false; error: string };
...
export function InboundWebhookCard({ initialToken }: InboundWebhookCardProps): React.JSX.Element {
  ...
  function onGenerate(): void {
    startTransition(async () => {
      ...
      if (!('ok' in result) || !result.ok) {
        toast.error(('error' in result && result.error) || 'Could not generate token.');
        return;
      }
      ...
      if ('token' in result && typeof result.token === 'string') {
        ...
        setRevealed(true);
        toast.success('Inbound token generated. Copy it now — you can reveal it again later.');
      } else {
  ...
  const displayValue = token ? (revealed ? token : maskInboundWebhookToken(token)) : null;

  return (
    <section className="space-y-4 rounded-xl border border-neutral-200 bg-white p-6 shadow-sm">
      <header className="space-y-1">
        <h2 className="text-lg font-semibold text-neutral-900">Inbound finding-status webhook</h2>
        <p className="text-sm text-neutral-600">
          Generate a token to let Slack, Linear, or any other tool POST a finding status change back
          into CarrierAudit. Send the token in the
      ...
      {displayValue ? (
        <div className="space-y-2">
          <div className="flex flex-col gap-2 rounded-lg border border-neutral-200 bg-neutral-50 p-3 sm:flex-row sm:items-center sm:justify-between">
            <code className="font-mono text-sm break-all text-neutral-900">{displayValue}</code>
          ...
          <p className="text-xs text-neutral-500">
            Anyone who holds this token can update findings on your account. Treat it like a
            password — rotate immediately if it leaks.
          </p>
        </div>
      ) : (
        <p className="text-sm text-neutral-700">
          You don&apos;t have an inbound token yet. Generate one to enable finding callbacks from
          external tools.
        </p>
      )}
      ...
      <div className="flex justify-end gap-2">
        {token ? (
          <Button type="button" size="sm" variant="outline" onClick={onRevoke} disabled={isPending}>
        ...
        <Button
          ...
          disabled={isPending}
        >
          {isPending ? 'Working…' : token ? 'Regenerate token' : 'Generate inbound token'}
        </Button>
      </div>
    </section>
  );
