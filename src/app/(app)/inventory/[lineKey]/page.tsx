...
export default async function InventoryLinePage({
  ...
  const minBase =
    baseValues.length > 0 ? baseValues.reduce((min, val) => Math.min(min, val), Infinity) : null;
  const maxBase =
    baseValues.length > 0 ? baseValues.reduce((max, val) => Math.max(max, val), -Infinity) : null;
  const deltaCents =
