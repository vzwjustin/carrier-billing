...
export default async function InventoryLinePage({
  ...
  // ⚡ Bolt: Single-pass iteration to calculate min/max without intermediate array allocations or spreads
  let minBase: number | null = null;
  let maxBase: number | null = null;
  let validBaseValuesCount = 0;
  for (const h of history) {
    if (typeof h.planBaseCents === 'number') {
      validBaseValuesCount++;
      if (minBase === null || h.planBaseCents < minBase) {
        minBase = h.planBaseCents;
      }
      if (maxBase === null || h.planBaseCents > maxBase) {
        maxBase = h.planBaseCents;
      }
    }
  }

  const deltaCents =
    validBaseValuesCount >= 2 && minBase !== null && maxBase !== null ? maxBase - minBase : null;
