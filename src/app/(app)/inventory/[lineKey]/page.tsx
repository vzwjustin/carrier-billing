...
export default async function InventoryLinePage({
  ...
  // Performance: Single-pass iteration to find min and max without intermediate arrays or spread operators
  let minBase: number | null = null;
  let maxBase: number | null = null;
  let baseCount = 0;

  for (const h of history) {
    const v = h.planBaseCents;
    if (typeof v === 'number') {
      baseCount++;
      if (minBase === null || v < minBase) minBase = v;
      if (maxBase === null || v > maxBase) maxBase = v;
    }
  }

  const deltaCents =
    baseCount >= 2 && minBase !== null && maxBase !== null ? maxBase - minBase : null;
