 ...
 *
 * The SSRF guard is stubbed so DNS doesn't run; global `fetch` is mocked so no
 * outbound request actually fires and we can assert exactly what the dispatcher
 * sends. The implementation intentionally uses normal fetch after preflight so
 * TLS SNI/certificate validation works for ordinary webhook endpoints.
 */
...
/** The arguments the dispatcher handed to mocked fetch. */
function lastReq(): FetchReq {
  const calls = fetchMock.mock.calls;
  expect(calls.length, 'fetch was not called').toBeGreaterThan(0);
  const [url, init] = calls[calls.length - 1]!;
  expect(init, 'fetch init was not provided').toBeTruthy();
  const reqInit = init as RequestInit;
  return {
    url: url as string,
    headers: reqInit.headers as Record<string, string>,
    body: reqInit.body as string,
  };
}
...
describe('postOutboundWebhook — SSRF preflight hand-off', () => {
  it('preflights the target then fetches the original hostname URL so TLS SNI/cert validate', async () => {
    await postOutboundWebhook({
      url: 'https://hook.example.com/path',
      secret: SECRET,
      audit: makeAudit(),
    ...
    const req = lastReq();
    expect(req.url).toBe('https://hook.example.com/path');
    expect(req.url).toContain('hook.example.com');
    expect(req.url).not.toContain('203.0.113.10');
  });

  it('fetches the original URL including port without forging Host', async () => {
    await postOutboundWebhook({
    ...
    const req = lastReq();
    expect(req.url).toBe('https://hook.example.com:8443/cb');
    expect(req.headers).not.toHaveProperty('Host');
  });
});
